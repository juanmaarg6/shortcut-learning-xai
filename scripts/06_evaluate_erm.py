"""Evaluate frozen ERM checkpoints once on the held-out Waterbirds test set."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the frozen ERM checkpoints on the held-out Waterbirds test set."
        )
    )
...
def load_frozen_checkpoint(
    ...
    """Load one already-selected ERM checkpoint without ImageNet re-download."""
...
def print_seed_metrics(
    ...
    """Print the final test metrics for one frozen checkpoint."""
    print(f"\nSeed {seed}")
    print("-" * 80)
...
def build_aggregate_table(
    ...
    """Compute mean and sample standard deviation across experimental seeds."""
    rows = []
...
def main() -> None:
    """Evaluate all frozen ERM seeds and save reproducible result artifacts."""
    args = parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    use_amp = device.type == "cuda"

    print("Final ERM test evaluation")
    print("=" * 80)
    ...
    for seed in args.seeds:
        ...
        prediction_dir = (
            PROJECT_ROOT / "results" / "predictions" / "erm" / f"seed_{seed}"
        )
        metric_dir = PROJECT_ROOT / "results" / "metrics" / "erm" / f"seed_{seed}"
    ...
    aggregate_dir = PROJECT_ROOT / "results" / "metrics" / "erm"
    aggregate_dir.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    print(
        "Per-example predictions: results/predictions/erm/seed_*/test_predictions.csv"
    )
    print("Per-seed test metrics:   results/metrics/erm/seed_*/test_metrics.json")
